...
from .mission_planner_core import (
    MissionPlannerCore, FlightPlan, Waypoint, MissionStatus,
    ValidationResult, ValidationIssue, SafetyConstraints
)
...
class MissionPlannerService:
    ...
    async def create_mission(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not operator_id or not uas_id or not waypoints_data:
                return {
                    'success': False,
                    'error': 'Missing required fields: operator_id, uas_id, waypoints'
                }
            ...
            mission = Mission(
                ...
                status=MissionStatus.DRAFT if validation_result == ValidationResult.WARNING else MissionStatus.VALIDATED,
    ...
    async def update_mission(self, mission_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if mission.status in [MissionStatus.ACTIVE, MissionStatus.COMPLETED]:
            return {
                'success': False,
                'error': f'Cannot update mission in status {mission.status.value}'
            }
        ...
        if 'waypoints' in updates or 'emergency_points' in updates:
            ...
            waypoints = [self._create_waypoint(wp) for wp in updates.get('waypoints', [])] or mission.flight_plan.waypoints
            emergency_points = [self._create_waypoint(ep) for ep in updates.get('emergency_points', [])] or mission.flight_plan.emergency_landing_points
            ...
            mission.status = MissionStatus.DRAFT if validation_result == ValidationResult.WARNING else MissionStatus.VALIDATED
    ...
    async def approve_mission(self, mission_id: str, approver_id: str) -> Dict[str, Any]:
        ...
        if not weather_ok:
            return {
                'success': False,
                'error': f'Weather conditions unsuitable: {weather_msg}'
            }
    ...
    async def complete_mission(self, mission_id: str, completion_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if mission.status != MissionStatus.ACTIVE:
            return {
                'success': False,
                'error': f'Mission is not active, current status: {mission.status.value}'
            }
    ...
    async def create_mission_from_template(self, template_id: str, 
    ...
    def _are_collinear(self, p1: Waypoint, p2: Waypoint, p3: Waypoint, 
                      tolerance: float = 0.01) -> bool:
        """Проверка, лежат ли три точки на одной линии"""
        # Упрощенная проверка через площадь треугольника
        area = abs((p2.latitude - p1.latitude) * (p3.longitude - p1.longitude) - 
    ...
    def _mission_to_dict(self, mission: Mission) -> Dict[str, Any]:
        ...
        result['flight_plan']['emergency_landing_points'] = [asdict(ep) for ep in mission.flight_plan.emergency_landing_points]
        return result
