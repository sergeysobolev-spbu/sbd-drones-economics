...
class MissionPlannerCore:
    ...
    def _load_no_fly_zones(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"id": "military-1", "center": {"lat": 55.8304, "lon": 37.5201}, "radius": 3000, "type": "military"},
        ]
            ...
            if waypoint.altitude > self.safety_constraints.max_altitude:
                issues.append(
                    ValidationIssue(
                        ...
                        severity="critical",
                        description=(
                            f"Waypoint {i}: altitude {waypoint.altitude}m exceeds maximum "
                            f"{self.safety_constraints.max_altitude}m"
                        ),
                        waypoint_index=i,
                    )
                )
            # Минимальная безопасная высота (не применяем предупреждение для "земли"/посадки)
                ...
                issues.append(
                    ValidationIssue(
                        ...
                        severity="warning",
                        description=(
                            f"Waypoint {i}: altitude {waypoint.altitude}m is below recommended minimum 10m"
                        ),
                        waypoint_index=i,
                    )
                )
            ...
            if waypoint.speed > self.safety_constraints.max_speed:
                issues.append(
                    ValidationIssue(
                        ...
                        severity="critical",
                        description=(
                            f"Waypoint {i}: speed {waypoint.speed}m/s exceeds maximum "
                            f"{self.safety_constraints.max_speed}m/s"
                        ),
                        waypoint_index=i,
                    )
                )
        ...
        for i, waypoint in enumerate(plan.waypoints):
            ...
            for zone in self.no_fly_zones:
                distance = self._calculate_distance(
                    waypoint.latitude, waypoint.longitude, zone["center"]["lat"], zone["center"]["lon"]
                )
                ...
                if distance < zone["radius"]:
                    issues.append(
                        ValidationIssue(
                            ...
                            severity="critical",
                            description=(
                                f"Waypoint {i} violates no-fly zone {zone['id']} ({zone['type']})"
                            ),
                            waypoint_index=i,
                        )
                    )
    ...
    def _validate_emergency_points(self, plan: FlightPlan) -> List[ValidationIssue]:
        ...
        if not plan.emergency_landing_points:
            ...
            for i, waypoint in enumerate(plan.waypoints):
                nearest_emergency = self._find_nearest_emergency_point(waypoint, plan.emergency_landing_points)
                ...
                if nearest_emergency > 500:  # 500м максимум до точки аварийной посадки
                    issues.append(
                        ValidationIssue(
                            ...
                            severity="warning",
                            description=(
                                f"Waypoint {i} is {nearest_emergency:.0f}m from nearest emergency landing point"
                            ),
                            waypoint_index=i,
                        )
                    )
    ...
    def _calculate_total_distance(self, waypoints: List[Waypoint]) -> float:
        ...
        for i in range(1, len(waypoints)):
            ...
            horizontal = self._calculate_distance(
                waypoints[i - 1].latitude, waypoints[i - 1].longitude, waypoints[i].latitude, waypoints[i].longitude
            )
    ...
    def _find_nearest_emergency_point(self, waypoint: Waypoint, emergency_points: List[Waypoint]) -> float:
        ...
        for emergency in emergency_points:
            distance = self._calculate_distance(
                waypoint.latitude, waypoint.longitude, emergency.latitude, emergency.longitude
            )
    ...
    def _check_spatial_conflicts(self, plan1: FlightPlan, plan2: FlightPlan) -> List[Dict[str, Any]]:
        ...
        for i, wp1 in enumerate(plan1.waypoints):
            for j, wp2 in enumerate(plan2.waypoints):
                distance = self._calculate_distance(wp1.latitude, wp1.longitude, wp2.latitude, wp2.longitude)

                altitude_diff = abs(wp1.altitude - wp2.altitude)

                if distance < min_separation and altitude_diff < 10:
                    conflicts.append({"waypoint": i, "other_waypoint": j, "distance": distance})
