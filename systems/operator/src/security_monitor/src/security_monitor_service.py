...
from .security_monitor_core import (
...
class SecurityMonitorService:
    ...
    async def process_request(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        rate_ok, rate_msg = self.core.check_rate_limit(
            sender_id, 
            context.action,
            self.request_history[sender_id]
        )
        ...
        if result == PolicyResult.DENY:
            ...
            return {
                'allowed': True,
                'audited': True,
                'warning': violation.details if violation else None
            }
        else:  # ALLOW
            return {
                'allowed': True,
    ...
    async def _handle_violation(self, context: SecurityContext, 
                              result: PolicyResult, violation: PolicyViolation):
    ...
    async def _send_notification(self, context: SecurityContext, 
                               violation: PolicyViolation, severity: str):
    ...
    def get_audit_log(self, start_time: Optional[float] = None, 
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        recent_requests = sum(
            ...
            for times in self.request_history.values()
        )
    ...
    async def cleanup_old_data(self):
        ...
        for sender_id in list(self.request_history.keys()):
            self.request_history[sender_id] = [
                t for t in self.request_history[sender_id] if t >= cutoff_time
            ]
    ...
    def _get_top_violated_policies(self, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        return [
            {'policy_id': policy_id, 'violations': count}
    ...
    def _get_high_risk_senders(self, threshold: int = 5) -> List[str]:
        ...
        return [
            sender_id for sender_id, count in sender_violations.items()
            if count >= threshold
        ]
    ...
    def _generate_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        if critical_violations > 0:
            recommendations.append(
                f"Обнаружено {critical_violations} критических нарушений. "
        ...
        if total > 0 and metrics['summary']['denied'] / total > 0.1:
            recommendations.append(
                "Более 10% запросов отклонено. "
                "Возможны проблемы с авторизацией или атака."
            )
