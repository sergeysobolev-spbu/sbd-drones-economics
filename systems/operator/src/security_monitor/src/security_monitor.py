...
from systems.operator.src.topics import (
    ComponentTopics, SecurityMonitorActions, SystemTopics
)
...
class SecurityMonitor(BaseComponent):
    ...
    async def _handle_check_policy(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'policy_id': policy_id,
            'result': 'allow',
            'details': f'Policy {policy_id} check passed'
        }
    ...
    async def _handle_audit_operation(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # В реальной системе запись сохранялась бы в защищенное хранилище
        ...
        while self._running:
            try:
                ...
                notification = await asyncio.wait_for(
                    self.service.notification_queue.get(),
                    timeout=1.0
                )
    ...
    async def _send_notification_to_regulator(self, notification: Dict[str, Any]):
        ...
        try:
            # Создаем сообщение для Регулятора
