...
class SecurityMonitorCore:
    ...
    def _load_security_policies(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "P3": {
                ...
                "rules": ["target_component in trusted_components", "sender_id in registered_components"],
            },
            "P4": {
                "id": "P4",
                "name": "Контроль частоты запросов",
                "description": "Защита от DoS атак",
                "rate_limits": {"default": 100, "critical_actions": 10},  # запросов в минуту
            },
        }
    ...
    def validate_request(self, context: SecurityContext) -> Tuple[PolicyResult, Optional[PolicyViolation]]:
        ...
        if not self._check_sender_authorization(context):
            violation = PolicyViolation(
                ...
                details=(
                    f"Sender {context.sender_id} with role {context.sender_role} "
                    f"not authorized for action {context.action}"
                ),
                severity="high",
                timestamp=time.time(),
            )
    ...
    def get_violations_summary(self) -> Dict[str, Any]:
        ...
        if not self.violations_log:
            return {"total_violations": 0, "by_severity": {}, "by_policy": {}}
        ...
        for violation in self.violations_log:
            ...
            if violation.policy_id not in by_policy:
                by_policy[violation.policy_id] = {"count": 0, "name": violation.policy_name}
        ...
        return {
            ...
            "by_policy": by_policy,
            "recent_violations": [
                {"policy": v.policy_id, "type": v.violation_type, "severity": v.severity, "timestamp": v.timestamp}
