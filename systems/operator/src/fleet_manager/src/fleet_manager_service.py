...
class FleetManagerService:
    ...
    async def initialize_fleet(self) -> Dict[str, Any]:
        ...
        try:
            # Прогреваем клиент регулятора (топики могут потребоваться в других сценариях)
            await self.regulator_client.get_system_topics()
    ...
    async def find_suitable_uas(self, requirements: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for uas_id, uas_ext in self._fleet_extended.items():
            ...
            suitability_score = self._calculate_suitability_score(uas_ext, core_state, requirements)
    ...
    async def reserve_uas_with_validation(
        self, uas_id: str, mission_id: str, operator_id: str, duration: int = 3600
    ) -> Dict[str, Any]:
        """
    ...
    async def release_uas_with_cleanup(self, uas_id: str, operator_id: str) -> Dict[str, Any]:
        """
        ...
        if success:
            ...
            return {"success": True, "uas_id": uas_id, "released_from": reservation_info.get("mission_id")}
        else:
    ...
    def _calculate_suitability_score(
        self, uas: UASExtended, state: Dict[str, Any], requirements: Dict[str, Any]
    ) -> float:
    ...
    def _group_purchases_by_developer(self, purchases: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for purchase in purchases:
            ...
            if dev_id not in by_developer:
                by_developer[dev_id] = {"count": 0, "total_spent": 0, "models": set()}
