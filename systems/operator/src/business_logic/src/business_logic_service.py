...
class MockTaxService:
    ...
    def calculate_tax(amount: float) -> Dict[str, Any]:
        ...
        return {"vat": round(vat, 2), "total_with_tax": round(amount + vat, 2)}
...
class BusinessLogicService:
    ...
    def create_proposal(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not margin_check.is_valid:
            return {"error": "Невозможно создать прибыльное предложение", "reason": margin_check.reason}
        ...
        return {"proposal": asdict(proposal), "cost_breakdown": asdict(cost_breakdown)}
    ...
    def process_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payment_result = self.banking.process_payment(
            {"amount": tax_info["total_with_tax"], "order_id": order_data.get("order_id")}
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if not self.proposals:
            return {"total_proposals": 0, "average_margin": 0, "total_value": 0}
