...
class BusinessLogicCore:
    ...
    def validate_discount(self, original_price: float, discounted_price: float) -> Dict[str, Any]:
        ...
        """
        if original_price <= 0:
            return {"valid": False, "reason": "Исходная цена должна быть положительной"}

        if discounted_price <= 0:
            return {"valid": False, "reason": "Цена со скидкой должна быть положительной"}
        ...
        return {"valid": True, "discount_percent": round(discount_percent, 2)}
    ...
    def validate_order_economics(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if has_discount:
            ...
            validations.append({"check": "discount", "passed": discount_check["valid"], "details": discount_check})
    ...
    def get_economic_limits(self) -> Dict[str, float]:
        ...
        """
        return {"min_margin_percent": self._MIN_MARGIN_PERCENT, "max_discount_percent": self._MAX_DISCOUNT_PERCENT}
