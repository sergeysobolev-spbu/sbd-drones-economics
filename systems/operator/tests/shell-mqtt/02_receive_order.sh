...
ORDER_ID="ORDER-MQTT-SHELL-$(date +%Y%m%d%H%M%S)"
...
echo "Publishing receive_order to ${REQ_TOPIC}"
echo "Waiting response on ${REPLY_TOPIC}"
RESP="$(mosq_request_reply "$REQ_TOPIC" "$REPLY_TOPIC" "$REQ_JSON" 20)"
if [[ -z "${RESP}" ]]; then
