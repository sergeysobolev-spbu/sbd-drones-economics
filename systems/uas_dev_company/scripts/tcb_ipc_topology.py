#!/usr/bin/env python3
"""Топология IPC между доменами ДВБ: дедупликация связей (отправитель→получатель) по политикам."""
...
def build_ipc_topology_task14(policy_rows: list[dict[str, str]]) -> dict:
    ...
    return {
        "task": "IPC-связи между доменами (дедуп по парам) и цели безопасности",
        "methodology_ru": (
