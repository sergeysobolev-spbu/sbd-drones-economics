...
e2e-codespace:
	...
	@echo "=== Generating multi-system compose ==="
	@$(LOAD_ENV) && pipenv run python scripts/prepare_multi.py --systems $(E2E_SYSTEMS) --output $(E2E_OUTPUT) $(E2E_PREPARE_ALT_FLAG)
	@echo "DELIVERY_DRONE_HEALTH_PORT=8095" >> $(E2E_OUTPUT)/.env
	@echo "AGRODRON_GATEWAY_HOST_PORT=18081" >> $(E2E_OUTPUT)/.env
	@echo "SYSTEM_MONITOR_HOST_PORT=18090" >> $(E2E_OUTPUT)/.env
	@$(LOAD_ENV) && echo "BROKER_USER=$${ADMIN_USER:-admin}" >> $(E2E_OUTPUT)/.env
	...
	@sleep $(E2E_WARMUP_SECONDS)
	@echo "=== Running E2E tests ==="
	@$(LOAD_ENV) && E2E_SKIP_ANALYTICS=1 E2E_ALT_INSURER='$(E2E_ALT_INSURER)' pipenv run python -m pytest tests/e2e/test_e2e_scenario.py -v -s $(E2E_PYTEST_ALT_FLAG) --tb=short 2>&1 || (echo "E2E tests failed"; $(E2E_COMPOSE_NO_ANALYTICS) --profile $(E2E_PROFILE) down -v 2>/dev/null; exit 1)
	@echo "=== Stopping E2E environment ==="
