...
def replace_insurer_with_alt_insurer(root: Path, systems: List[str]) -> List[str]:
    """
    Подмена insurer → alt_insurer для единого compose.
    Дедупликация по каноническому пути системы.
    """
    out: List[str] = []
    seen: set[str] = set()
    for item in systems:
        raw = item.strip()
        path = normalize_system_path(root, raw)
        if path.name.lower() == "insurer":
            raw = "systems/alt_insurer" if "systems/" in raw.replace("\\", "/") else "alt_insurer"
            path = normalize_system_path(root, raw)
...
def prepare_multi(
    ...
    output: Optional[str],
    *,
    use_alt_insurer: bool = False,
) -> None:
    root = Path(__file__).resolve().parent.parent
    if use_alt_insurer:
        systems = replace_insurer_with_alt_insurer(root, systems)
    ...
    for sys_path in system_paths:
        ...
        for original_name, svc in sys_services.items():
            ...
            # Operator → страховая: по умолчанию systems.insurer; для alt_insurer см. --alt-insurer.
            if use_alt_insurer and sys_name.lower() == "operator" and original_name in (
                "operator_component",
                "operator_gateway",
            ):
                env_dict["EXTERNAL_INSURER_TOPIC"] = "systems.alt_insurer"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--alt-insurer",
        action="store_true",
        help="Заменить systems/insurer на alt_insurer; EXTERNAL_INSURER_TOPIC для Operator.",
    )
...
if __name__ == "__main__":
    try:
        args = parse_args()
        prepare_multi(args.systems, args.output, use_alt_insurer=args.alt_insurer)
