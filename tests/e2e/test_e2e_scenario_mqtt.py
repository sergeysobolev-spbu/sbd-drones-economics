"""
E2E (MQTT transport): full business flow + health checks + analytics.

Mirrors test_e2e_scenario.py but drives the scenario over MQTT via the
`mqtt_bus` fixture from conftest.py. All Python/Java systems must run
with BROKER_TYPE=mqtt; Agregator (Go) stays on Kafka + OPERATOR_TRANSPORT=both
so MQTT operator bridging works.
...
# Shared state across ordered test classes (isolated from the Kafka scenario).
...
class Test0_SystemsInRegulator:
    ...
    def test_register_systems(self, mqtt_bus):
        for system_id, system_type in (
            ...
            resp = bus_request(mqtt_bus, REGULATOR_TOPIC, "register_system", {
                "system_id": system_id,
        ...
        v = bus_request(mqtt_bus, REGULATOR_TOPIC, "verify_system", {"system_id": "operator"})
...
class Test1_DroneRegistration:
    ...
    def test_01_register_drone_cert(self, mqtt_bus):
        r = bus_request(mqtt_bus, REGULATOR_TOPIC, "register_drone_cert", {
            "drone_id": self.DRONE_ID,
        })
        assert r.get("success") is True
        _shared["drone_cert_id"] = (r.get("payload") or {})["certificate_id"]

    def test_02_register_drone_at_operator(self, mqtt_bus):
        r = bus_request(mqtt_bus, OPERATOR_TOPIC, "register_drone", {
            "drone_id": self.DRONE_ID,
            "model": "AgroDron-X1",
    ...
    def test_03_register_drone_in_orvd(self, mqtt_bus):
        r = bus_request(mqtt_bus, OPERATOR_TOPIC, "register_drone_in_orvd", {
            "drone_id": self.DRONE_ID,
            "model": "AgroDron-X1",
            "certificate_id": _shared["drone_cert_id"],
        })
    ...
    def test_04_register_drone_in_droneport(self, mqtt_bus):
        ...
        resp = bus_request(mqtt_bus, DRONE_PORT_REGISTRY_TOPIC, "get_drone", {
            "drone_id": self.DRONE_ID,
        })
        if resp.get("success"):
            pl = resp.get("payload") or {}
    ...
    def test_05_annual_insurance(self, mqtt_bus):
        ...
        assert r is not None, (
            ...
            "проверьте логи insurer и MQTT (mosquitto, SPRING_PROFILES_ACTIVE=mqtt)"
        )
...
class Test2_OperatorInAggregator:
    ...
    def test_01_register_operator_cert(self, mqtt_bus):
        r = bus_request(mqtt_bus, REGULATOR_TOPIC, "register_operator_cert", {
    ...
    def test_03_verify_operator_cert(self, mqtt_bus):
        v = bus_request(mqtt_bus, REGULATOR_TOPIC, "verify_operator_cert", {
            "operator_id": "e2e-operator-1",
            "certificate_id": _shared["operator_cert_id"],
        })
...
class Test3_OrderFlow:
    ...
    Agregator (Go) публикует create_order в Kafka. Operator под MQTT слушает
    MQTT-топик operator.requests: Agregator с OPERATOR_TRANSPORT=both дублирует
    публикацию в MQTT, поэтому matching работает.
    """
    ...
    def test_02_create_order_and_wait_for_match(self, agregator_url, mqtt_bus):
        ...
        # Fallback: republish to MQTT operator.requests directly if matching lags.
        if status != "matched":
            mqtt_bus.publish(
        ...
        if status != "matched":
            pytest.skip(
                ...
                "Operator↔Agregator MQTT bridge may still be unavailable"
            )
...
class Test4_MissionPlanning:
    ...
    def test_01_register_mission_orvd(self, mqtt_bus):
    ...
    def test_02_authorize_mission_orvd(self, mqtt_bus):
    ...
    def test_03_gcs_plan_route(self, mqtt_bus):
        ...
        r = bus_request(mqtt_bus, GCS_ORCHESTRATOR_TOPIC, "task.submit", {
            "waypoints": [
...
class Test5_SystemHealthChecks:
    ...
    def test_droneport_ping(self, mqtt_bus):
        try:
            resp = bus_request(mqtt_bus, DRONE_PORT_REGISTRY_TOPIC, "ping", {}, timeout=10)
            assert resp.get("success") is True
        except AssertionError:
            pytest.skip("DronePort registry not reachable — container may not be running")

    def test_agrodron_ping(self, mqtt_bus):
        try:
            resp = bus_request(mqtt_bus, AGRODRON_SECURITY_MONITOR_TOPIC, "ping", {}, timeout=10)
            assert resp.get("success") is True
        except AssertionError:
            pytest.skip("AgroDron security_monitor not reachable — container may not be running")

    def test_gcs_orchestrator_ping(self, mqtt_bus):
        try:
            resp = bus_request(mqtt_bus, GCS_ORCHESTRATOR_TOPIC, "ping", {}, timeout=10)
            assert resp.get("success") is True
        except AssertionError:
            pytest.skip("GCS orchestrator not reachable — container may not be running")

    def test_sitl_telemetry_request(self, mqtt_bus):
        deadline = time.time() + 90
        last_resp = None
        while time.time() < deadline:
            resp = mqtt_bus.request(
...
class Test6_MissionExecution:
    """Full mission execution cycle over MQTT."""

    DRONE_ID = E2E_DRONE_ID

    def test_01_gcs_task_assign(self, mqtt_bus):
    ...
    def test_02_gcs_task_start(self, mqtt_bus):
    ...
    def test_03_poll_autopilot_state(self, mqtt_bus):
    ...
    def test_04_wait_mission_completed(self, mqtt_bus):
        if not _shared.get("mission_started"):
